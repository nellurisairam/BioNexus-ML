...
Model: LinearRegression inside an sklearn Pipeline with median imputation + scaling.

Subcommands:
  train     - Train a LinearRegression pipeline and save model + schema
  predict   - Predict Product_Titer_gL for a new CSV (unlabeled)
  benchmark - Evaluate a saved model on a labeled CSV (R^2, MAE, RMSE)

Examples:
  python predict.py train --data bioreactor_ml_dataset.csv --target Product_Titer_gL --out outputs/model_linear.joblib
  python predict.py predict --model outputs/model_linear.joblib --schema outputs/feature_schema.json --input new_samples.csv --output outputs/predictions.csv
  python predict.py benchmark --model outputs/model_linear.joblib --schema outputs/feature_schema.json --data bioreactor_ml_dataset.csv --target Product_Titer_gL
"""
...
DEFAULT_MODEL = 'outputs/model_linear.joblib'
...
def build_parser():
    p = argparse.ArgumentParser(description='Bioreactor ML deploy script (LinearRegression model): train / predict / benchmark')
    sub = p.add_subparsers(dest='cmd', required=True)

    # train
    p_train = sub.add_parser('train', help='Train a LinearRegression pipeline and save artifact')
